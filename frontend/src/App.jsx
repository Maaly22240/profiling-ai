...
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.error) return (
      <div style={{ minHeight:'100vh', background:'#04091A', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'monospace', padding:'40px' }}>
        <div style={{ maxWidth:'700px', background:'rgba(239,68,68,0.08)', border:'1px solid rgba(239,68,68,0.3)', borderRadius:'12px', padding:'28px' }}>
          <p style={{ color:'#fca5a5', fontSize:'16px', fontWeight:'700', marginBottom:'12px' }}>💥 Erreur détectée</p>
          <pre style={{ color:'#fca5a5', fontSize:'12px', whiteSpace:'pre-wrap', wordBreak:'break-word' }}>{this.state.error?.message}</pre>
          <pre style={{ color:'rgba(252,165,165,0.5)', fontSize:'11px', marginTop:'12px', whiteSpace:'pre-wrap', wordBreak:'break-word' }}>{this.state.error?.stack?.slice(0,800)}</pre>
          <button onClick={() => window.location.reload()} style={{ marginTop:'16px', padding:'8px 18px', background:'rgba(239,68,68,0.15)', border:'1px solid rgba(239,68,68,0.3)', borderRadius:'7px', color:'#fca5a5', fontFamily:'monospace', cursor:'pointer' }}>
            ↺ Recharger
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ position:'relative' }}>
      <button onClick={() => setOpen(o => !o)} style={{
        display:'flex', alignItems:'center', gap:'9px', padding:'5px 12px 5px 5px',
        ...
        borderRadius:'10px', cursor:'pointer', transition:'all 0.2s', fontFamily:'inherit',
      }}>
        <div style={{ width:'28px', height:'28px', borderRadius:'8px', background:'linear-gradient(135deg,#E87722,#1B3568)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'11px', fontWeight:'700', color:'white', flexShrink:0 }}>{initials}</div>
      ...
      {open && (
        <>
          ...
          <div style={{ position:'absolute', top:'calc(100% + 8px)', right:0, zIndex:199, background:'rgba(4,9,26,0.98)', border:'1px solid rgba(44,123,229,0.2)', borderRadius:'14px', minWidth:'220px', boxShadow:'0 16px 48px rgba(0,0,0,0.6)', overflow:'hidden', animation:'dropIn 0.15s ease both' }}>
            <div style={{ padding:'14px 16px 12px', borderBottom:'1px solid rgba(44,123,229,0.1)' }}>
              <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'8px' }}>
                <div style={{ width:'36px', height:'36px', borderRadius:'10px', background:'linear-gradient(135deg,#E87722,#1B3568)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'14px', fontWeight:'700', color:'white' }}>{initials}</div>
                <div>
              ...
              {expiresAt && (
                <div style={{ display:'flex', alignItems:'center', gap:'6px', padding:'5px 8px', background:'rgba(27,75,154,0.06)', border:'1px solid rgba(44,123,229,0.1)', borderRadius:'6px' }}>
                  <span style={{ fontSize:'10.5px', color:'#4A6A96' }}>Expire à <strong style={{ color:'#607CA8' }}>{expiresAt}</strong></span>
                </div>
              )}
            </div>
            <div style={{ padding:'6px' }}>
              <button onClick={() => { setOpen(false); onProfile(); }} style={{ width:'100%', padding:'8px 12px', background:'transparent', border:'none', borderRadius:'8px', cursor:'pointer', display:'flex', alignItems:'center', gap:'10px', transition:'background 0.15s', fontFamily:'inherit', textAlign:'left' }}
                onMouseEnter={e => e.currentTarget.style.background='rgba(44,123,229,0.08)'}
                onMouseLeave={e => e.currentTarget.style.background='transparent'}
              >
                <span style={{ width:'28px', height:'28px', borderRadius:'7px', background:'rgba(27,75,154,0.1)', border:'1px solid rgba(44,123,229,0.12)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'13px' }}>👤</span>
                <div>
                  <p style={{ fontSize:'12.5px', fontWeight:'500', color:'#e2e8f0', margin:0 }}>Mon profil</p>
                  <p style={{ fontSize:'10.5px', color:'#4A6A96', margin:0 }}>Compte & sécurité</p>
                </div>
              </button>
              <div style={{ height:'1px', background:'rgba(239,68,68,0.1)', margin:'4px 0' }}/>
              <button onClick={() => { setOpen(false); onLogout(); }} style={{ width:'100%', padding:'9px 12px', background:'transparent', border:'none', borderRadius:'8px', cursor:'pointer', display:'flex', alignItems:'center', gap:'10px', transition:'background 0.15s', fontFamily:'inherit', textAlign:'left' }}
                onMouseEnter={e => e.currentTarget.style.background='rgba(239,68,68,0.1)'}
                onMouseLeave={e => e.currentTarget.style.background='transparent'}
              >
                <span style={{ width:'28px', height:'28px', borderRadius:'7px', background:'rgba(239,68,68,0.08)', border:'1px solid rgba(239,68,68,0.15)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'13px' }}>⎋</span>
                <div>
...
function App() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      verifyToken().then(serverUser => {
        if (serverUser) { setUser(serverUser); setIsLoggedIn(true); setPage('dashboard'); }
  ...
  useEffect(() => {
    const handler = (e) => {
      setIsLoggedIn(false); setUser(null); setPage('home');
      setImportedData({ columns:[], rows:[] });
      setSessionMsg(e.detail?.reason || 'Session expirée.');
  ...
  const handleLogin   = (u) => { setUser(u); setIsLoggedIn(true); setPage('dashboard'); setSessionMsg(''); };
  const handleLogout  = () => { logout(); setIsLoggedIn(false); setUser(null); setPage('home'); setImportedData({ columns:[], rows:[] }); };
  ...
  const PAGE_LABELS = { home:'Accueil', import:'Import', dashboard:'Dashboard', pareto:'Pareto', profile:'Profil' };
  const breadcrumb  = ['import','dashboard','pareto','profile'].slice(0, ['import','dashboard','pareto','profile'].indexOf(page)+1).filter(() => isLoggedIn);
  ...
  return (
    <ErrorBoundary>
      <div style={{ minHeight:'100vh', background:'linear-gradient(135deg,#04091A 0%,#060E22 50%,#04091A 100%)', fontFamily:"'DM Sans','Segoe UI',sans-serif" }}>
        ...
        {/* Toast */}
        {sessionMsg && (
          <div style={{ position:'fixed', bottom:'24px', right:'24px', zIndex:300, padding:'12px 18px', background:'rgba(239,68,68,0.12)', border:'1px solid rgba(239,68,68,0.35)', borderRadius:'10px', color:'#fca5a5', fontSize:'13px', fontWeight:'500', backdropFilter:'blur(12px)', display:'flex', alignItems:'center', gap:'10px', fontFamily:'inherit' }}>
        ...
        <nav style={{ position:'sticky', top:0, zIndex:100, borderBottom:'1px solid rgba(44,123,229,0.15)', backdropFilter:'blur(20px)', background:'rgba(4,9,26,0.88)', padding:'0 40px', display:'flex', alignItems:'center', justifyContent:'space-between', height:'64px' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'12px' }}>
            <div onClick={() => setPage(isLoggedIn?'dashboard':'home')} style={{ display:'flex', alignItems:'center', gap:'10px', cursor:'pointer' }}>
              ...
              <span style={{ fontSize:'17px', fontWeight:'700', letterSpacing:'-0.3px', color:'#f0f4ff' }}>Profiling<span style={{ color:'#E87722', fontWeight:'900' }}>.</span>ai</span>
            </div>
            {isLoggedIn && breadcrumb.length > 0 && (
              <div style={{ display:'flex', alignItems:'center', gap:'5px', marginLeft:'8px' }}>
                {breadcrumb.map((p,i) => (
                  <span key={p} style={{ display:'flex', alignItems:'center', gap:'5px' }}>
                    ...
                    <span onClick={() => setPage(p)} style={{ fontSize:'12px', color:p===page?'#FFA94D':'#2E4A72', cursor:p===page?'default':'pointer', fontWeight:p===page?'600':'400' }}>
                      {PAGE_LABELS[p]}
                    </span>
                  </span>
                ))}
          ...
          <div style={{ display:'flex', alignItems:'center', gap:'12px' }}>
            ...
            {isLoggedIn && (
              <div style={{ display:'flex', gap:'3px' }}>
                {[{id:'dashboard',label:'Dashboard',icon:'📊'},{id:'import',label:'Import',icon:'📥'},{id:'pareto',label:'Pareto',icon:'📈'}].map(tab => (
            ...
            {isLoggedIn && user && (
              <ProfileMenu user={user} onLogout={handleLogout} onProfile={() => setPage('profile')}/>
            )}
          </div>
        </nav>
