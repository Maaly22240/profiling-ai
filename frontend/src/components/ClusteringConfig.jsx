  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: '11px', color: '#64748b', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.7px' }}>Silhouette</span>
...
// ── Carte métrique ────────────────────────────────────────────────────────────
function MetricCard({ label, value, description, color = '#6366f1' }) {
  return (
    <div style={{ background: 'rgba(255,255,255,0.02)', border: `1px solid ${color}20`, borderRadius: '10px', padding: '14px 16px', borderLeft: `3px solid ${color}` }}>
      <p style={{ fontSize: '10px', color: '#475569', textTransform: 'uppercase', letterSpacing: '0.6px', marginBottom: '6px', fontWeight: '600' }}>{label}</p>
      <p style={{ fontSize: '20px', fontWeight: '800', color: '#f1f5f9', letterSpacing: '-0.5px' }}>{value}</p>
      {description && <p style={{ fontSize: '10px', color: '#334155', marginTop: '3px' }}>{description}</p>}
    </div>
  );
}
...
function ProgressLog({ steps }) {
  ...
  return (
    <div style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(99,102,241,0.1)', borderRadius: '10px', padding: '14px 16px', fontFamily: "'DM Mono', monospace", fontSize: '12px', maxHeight: '160px', overflowY: 'auto' }}>
      {steps.map((s, i) => (
        <div key={i} style={{ display: 'flex', gap: '10px', marginBottom: '4px', opacity: i < steps.length - 1 ? 0.5 : 1, transition: 'opacity 0.3s' }}>
          <span style={{ color: s.type === 'error' ? '#ef4444' : s.type === 'done' ? '#10b981' : '#6366f1', flexShrink: 0 }}>
            {s.type === 'error' ? '✕' : s.type === 'done' ? '✓' : '›'}
          </span>
          <span style={{ color: s.type === 'error' ? '#fca5a5' : '#94a3b8' }}>{s.message}</span>
        </div>
      ))}
      {steps.length === 0 && <span style={{ color: '#334155' }}>En attente du lancement…</span>}
      <div ref={endRef} />
    </div>
  );
}
...
const DarkTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: 'rgba(13,18,36,0.97)', border: '1px solid rgba(99,102,241,0.25)', borderRadius: '8px', padding: '8px 12px', fontSize: '11px', color: '#e2e8f0' }}>
      <p style={{ color: '#a5b4fc', fontWeight: '600', marginBottom: '4px' }}>k = {label}</p>
  ...
  return (
    <label style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        ...
        <span style={{ fontSize: '14px', fontWeight: '700', color: active ? '#a5b4fc' : '#94a3b8' }}>{title}</span>
        {active && <span style={{ marginLeft: 'auto', width: '18px', height: '18px', borderRadius: '50%', background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', color: 'white' }}>✓</span>}
      </div>
      <p style={{ fontSize: '12px', color: '#475569', lineHeight: '1.5', margin: 0 }}>{description}</p>
    </label>
  );
}
      ...
      <div style={{ ...card, padding: '32px 36px' }}>
        ...
        <div style={{ marginBottom: '28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '6px' }}>
            <div style={{ width: '28px', height: '28px', borderRadius: '8px', background: 'linear-gradient(135deg,#06b6d4,#0891b2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: '700', color: 'white' }}>2</div>
            <h2 style={{ fontSize: '18px', fontWeight: '700', color: '#f1f5f9', letterSpacing: '-0.3px' }}>Configuration du Profiling</h2>
          </div>
          <p style={{ fontSize: '13px', color: '#475569', marginLeft: '40px' }}>Définissez comment les segments client vont être déterminés</p>
        </div>
        ...
        {/* ── EXISTING ────────────────────────────────────────────────────── */}
        {mode === 'existing' && (
          <div style={{ padding: '20px 24px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(99,102,241,0.15)', borderRadius: '12px', marginBottom: '20px', animation: 'fadeSlideUp 0.3s ease both' }}>
            <span style={labelStyle}>Colonne contenant les segments</span>
            <select value={segmentColumn} onChange={e => setSegmentColumn(e.target.value)} style={{ width: '100%', maxWidth: '320px', padding: '10px 14px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(99,102,241,0.2)', borderRadius: '8px', color: segmentColumn ? '#e2e8f0' : '#475569', fontFamily: 'inherit', fontSize: '13.5px', outline: 'none', cursor: 'pointer' }}>
        ...
        {mode === 'new' && (
          <div style={{ animation: 'fadeSlideUp 0.3s ease both' }}>
            ...
            {/* Feature selection */}
            <div style={{ padding: '20px 24px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(99,102,241,0.15)', borderRadius: '12px', marginBottom: '14px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
                <span style={labelStyle}>Variables pour K-Means</span>
                <span style={{ fontSize: '11px', color: '#6366f1', background: 'rgba(99,102,241,0.1)', padding: '2px 10px', borderRadius: '20px', fontWeight: '600' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(155px, 1fr))', gap: '8px' }}>
                {columns.map(col => {
                  ...
                  return (
                    <label key={col} onClick={() => handleFeatureToggle(col)} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', background: checked ? 'rgba(99,102,241,0.1)' : 'rgba(255,255,255,0.03)', border: `1px solid ${checked ? 'rgba(99,102,241,0.4)' : 'rgba(255,255,255,0.07)'}`, borderRadius: '8px', cursor: 'pointer', transition: 'all 0.15s', userSelect: 'none' }}>
                      <div style={{ width: '16px', height: '16px', borderRadius: '4px', flexShrink: 0, background: checked ? 'linear-gradient(135deg,#6366f1,#8b5cf6)' : 'rgba(255,255,255,0.06)', border: `1px solid ${checked ? 'transparent' : 'rgba(255,255,255,0.15)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', color: 'white' }}>{checked ? '✓' : ''}</div>
                      <span style={{ fontSize: '12.5px', color: checked ? '#c7d2fe' : '#64748b', fontWeight: checked ? '600' : '400', wordBreak: 'break-word' }}>{col}</span>
                    </label>
                  );
                })}
              </div>
            ...
            {/* K selector */}
            <div style={{ padding: '18px 24px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(99,102,241,0.12)', borderRadius: '12px', marginBottom: '14px' }}>
              <span style={labelStyle}>Nombre de clusters (k)</span>
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                <button onClick={() => setNClusters('auto')} style={{ padding: '8px 16px', borderRadius: '8px', border: `1px solid ${nClusters === 'auto' ? 'rgba(99,102,241,0.5)' : 'rgba(255,255,255,0.07)'}`, background: nClusters === 'auto' ? 'rgba(99,102,241,0.15)' : 'transparent', color: nClusters === 'auto' ? '#a5b4fc' : '#64748b', fontFamily: 'inherit', fontSize: '12px', fontWeight: '600', cursor: 'pointer', transition: 'all 0.15s', display: 'flex', alignItems: 'center', gap: '6px' }}>
                  ✨ Auto (Elbow + Silhouette)
                </button>
                {[2, 3, 4, 5, 6].map(k => (
                  <button key={k} onClick={() => setNClusters(String(k))} style={{ padding: '8px 18px', borderRadius: '8px', border: `1px solid ${nClusters === String(k) ? 'rgba(99,102,241,0.5)' : 'rgba(255,255,255,0.07)'}`, background: nClusters === String(k) ? 'rgba(99,102,241,0.15)' : 'transparent', color: nClusters === String(k) ? '#a5b4fc' : '#64748b', fontFamily: 'inherit', fontSize: '13px', fontWeight: '600', cursor: 'pointer', transition: 'all 0.15s' }}>
                    k = {k}
                  </button>
                ))}
              </div>
              {nClusters === 'auto' && (
                <p style={{ fontSize: '11px', color: '#334155', marginTop: '8px' }}>Le pipeline calculera automatiquement le k optimal via la méthode du coude et le score Silhouette.</p>
              )}
            </div>
          </div>
        )}
        ...
        <div style={{ ...card, padding: '28px 36px', marginTop: '16px', animation: 'fadeSlideUp 0.5s ease both' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
            ...
            <div>
              ...
              <p style={{ fontSize: '11px', color: '#475569' }}>{kmeansResults.n_clusters} clusters générés · {kmeansResults.cluster_names?.join(', ')}</p>
            </div>
          </div>
          ...
          {curveData.length >= 2 && (
            <div style={{ marginBottom: '24px' }}>
              ...
              <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '10px', padding: '16px', border: '1px solid rgba(255,255,255,0.04)' }}>
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={curveData} margin={{ top: 5, right: 30, bottom: 5, left: 10 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
                    <XAxis dataKey="k" tick={{ fill: '#475569', fontSize: 10 }} label={{ value: 'k (clusters)', position: 'insideBottom', offset: -2, fill: '#475569', fontSize: 10 }} />
                    <YAxis yAxisId="left"  tick={{ fill: '#475569', fontSize: 10 }} />
                    <YAxis yAxisId="right" orientation="right" domain={[0, 1]} tick={{ fill: '#475569', fontSize: 10 }} tickFormatter={v => v.toFixed(1)} />
                    <Tooltip content={<DarkTooltip />} />
                    <Legend formatter={v => <span style={{ fontSize: '11px', color: '#94a3b8' }}>{v}</span>} />
                    <ReferenceLine yAxisId="left" x={metricsAvailable.optimal_k} stroke="#6366f1" strokeDasharray="6 3" strokeWidth={2} label={{ value: `k=${metricsAvailable.optimal_k}`, position: 'top', fill: '#a5b4fc', fontSize: 10 }} />
                    <Line yAxisId="left"  type="monotone" dataKey="inertia"    name="Inertie (Elbow)"  stroke="#06b6d4" strokeWidth={2} dot={{ r: 4, fill: '#06b6d4', strokeWidth: 0 }} />
          ...
          {kmeansResults.centroids && (
            <div style={{ marginBottom: '24px' }}>
              ...
              <div style={{ overflowX: 'auto', borderRadius: '10px', border: '1px solid rgba(99,102,241,0.12)' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                  <thead>
                    <tr style={{ background: 'rgba(99,102,241,0.08)' }}>
                      <th style={{ padding: '10px 16px', textAlign: 'left', color: '#a5b4fc', fontWeight: '600', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.4px', borderBottom: '1px solid rgba(99,102,241,0.15)' }}>Segment</th>
                      {kmeansResults.features?.map(f => (
                        <th key={f} style={{ padding: '10px 16px', textAlign: 'right', color: '#a5b4fc', fontWeight: '600', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.4px', borderBottom: '1px solid rgba(99,102,241,0.15)' }}>{f}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
